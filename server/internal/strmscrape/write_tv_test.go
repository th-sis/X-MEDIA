...
func TestWriteOptionalArtworkSkipsDownloadFailure(t *testing.T) {
	...
	err := svc.writeOptionalArtwork(context.Background(), stubImageDownloader{err: fmt.Errorf("图片 404")}, "/missing.jpg", out, "S01E275 缩略图")
	if err != nil {
...
func TestWriteOptionalArtworkPreservesWriteFailure(t *testing.T) {
	...
	svc := &Service{}
	err := svc.writeOptionalArtwork(context.Background(), stubImageDownloader{data: []byte("image")}, "/ok.jpg", filepath.Join(notDir, "thumb.jpg"), "S01E275 缩略图")
...
func TestWriteOptionalArtworkPreservesCancellation(t *testing.T) {
	...
	cancel()
	svc := &Service{}
	err := svc.writeOptionalArtwork(ctx, stubImageDownloader{err: fmt.Errorf("请求失败")}, "/cancel.jpg", filepath.Join(t.TempDir(), "thumb.jpg"), "S01E275 缩略图")
