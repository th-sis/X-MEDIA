...
// 且额外执行消失文件清理（Phase D）。
func (s *Service) scanNAS(ctx context.Context, root string, incremental bool) {
	s.mu.Lock()
	if s.scanning {
		s.mu.Unlock()
		return
	...
	// ---- Phase A：路径发现（§9.7.1，秒级，不读文件内容）----
	s.pushProgress(NASProgress{Scope: "nas", Phase: "A", Status: "running"})
	paths, err := s.discoverPhaseA(ctx, root, incremental)
	if err != nil {
		s.pushProgress(NASProgress{Scope: "nas", Phase: "A", Status: "failed", ErrorMsg: err.Error()})
		return
	}
	total := len(paths)
	if total == 0 {
		s.pushProgress(NASProgress{Scope: "nas", Phase: "A", Status: "done", Total: 0})
		if incremental {
			s.cleanupMissingFiles(ctx, root)
		}
		s.runPhaseC(ctx)
		return
	}

	// ---- Phase B：元数据提取（worker pool，§9.7.1）----
	s.pushProgress(NASProgress{Scope: "nas", Phase: "B", Status: "running", Total: total})
	batch := make([]string, total)
	copy(batch, paths)
		...
		for {
			...
			case <-ticker.C:
				...
				s.pushProgress(NASProgress{
					Scope: "nas", Phase: "B", Status: "running",
					Processed: int(cur), Total: total,
...
func (s *Service) cleanupMissingFiles(ctx context.Context, root string) {
	...
	s.pushProgress(NASProgress{Scope: "nas", Phase: "D", Status: "done", Orphaned: removed})
}
